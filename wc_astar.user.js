...
(function() {
	function source() {
		(function astar_module() {
			...
			Node.prototype.initSurfaceType = function() {
				var bg, res = /(\d+).gif/;
				if (this.cell.hasAttribute("background")) {
					bg = this.cell.getAttribute("background").match(res)[1];
				} else {
					bg = this.cell.childNodes[0].getAttribute("src").match(res)[1];
				}
				if (bg >= 48 && bg <=58)
					this.terrainType = 1;
				else if (bg >= 96 && bg <= 110)
					this.terrainType = 2
				else if (bg >=64 && bg <= 78)
					this.terrainType = 3;
				else if (bg >= 80 && bg <= 84)
					this.terrainType = 4;
				else if (bg >= 112 && bg <= 119)
					this.terrainType = 5;
				else if (bg >= 32 && bg <= 47)
					this.terrainType = 6;
			}
			...
			Node.prototype.getAdjCell = function(dx, dy) {
				if ((this.i + dy > 0) && (this.i + dy < this.map.rows.length)
					...
					return this.map.rows[this.i+dy].cells[this.j+dx];
				}
				return null;
			}
			Node.prototype.getAdjCells = function() {
				...
				for (var i = -1; i <= 1; i++)
					for (var j = -1; j <= 1; j++) {
						var adjCell = this.getAdjCell(i, j);
						if (adjCell != null)
...
/*
		...
		*/
			Node.prototype.checkForEnemyOnCell = function(cell) {
				if (cell.childNodes.length == 3
						&& cell.childNodes[2].hasAttribute("class")
						&& cell.childNodes[2].getAttribute("class") == "ind4") {
						return true;
					}
				return false;
			}
			Node.prototype.checkForEnemyOnAdjCell = function() {
				...
				if (this.enemyOnAdjCell == null) {
					for (var i = 0; i < adjCells.length; i++) {
						if (this.checkForEnemyOnCell(adjCells[i])) {
			...
			*/
			...
			Node.prototype.initCost = function(activeUnit) {
				var bg, obj = null,
				cult = sessionStorage.getItem("cult") == null ? "" : sessionStorage.getItem("cult"),
				res = /(\d+).gif/;
				if (this.cell.hasAttribute("background")) {
					bg = this.cell.getAttribute("background").match(res)[1];
					obj = this.cell.childNodes[0].getAttribute("src").match(res)[1];
				} else {
					bg = this.cell.childNodes[0].getAttribute("src").match(res)[1];
				}
				if (parseInt(activeUnit / 1000) == 9 && parseInt(activeUnit % 10) == 0 ||
					...
					// own town
					if (obj != null && this.cell.childNodes[0].hasAttribute("style") && (obj == 23 || obj == 13 || obj == 3))
						this.cost = 0.5;
			...
			Node.prototype.initAccessible = function(activeUnit) {
				var bg, obj = null,
				cult = sessionStorage.getItem("cult") == null ? "" : sessionStorage.getItem("cult"),
				res = /(\d+).gif/;
				if (this.cell.hasAttribute("background")) {
				...
				if (parseInt(activeUnit / 1000) == 9 && parseInt(activeUnit % 10) == 0 ||
					...
					if (bg >= 32 && bg <= 47) {
						...
						if (this.cell.childNodes[0].hasAttribute("style") && (obj == 23 || obj == 13 || obj == 3))
							this.accessible = 1;
						else if (obj == 206 || obj == 56 || obj == 76)
							this.accessible = 1;
						else
							this.accessible = 0;
					} else {
						this.accessible = 1;
								   ...
								   activeUnit == 9232 || activeUnit == 9052) {
					if (obj == null && bg >= 32 && bg <= 47) {
						this.accessible = 1;
					} else if (obj == 206 || obj == 56 || obj == 76) {
						var m = this.cell.childNodes[0].getAttribute("tooltip").match(/<i>(.*)<\/i>/);
						if (m && m[1].search("кря") != -1) {
							this.accessible = 0;
						} else
							this.accessible = 1;
					} else {
			...
			function astar(destX, destY) {
				...
				for (var i = 0; i < map.rows.length; i++) {
					...
					for (var j = 0; j < map.rows[i].cells.length; j++) {
						...
						node.initSurfaceType();
						node.initCost(activeUnit);
						node.initAccessible(activeUnit);
